...
pub struct TokioDelayFunc<F: Future> {
  handle: Option<DelayFuncHandle<F>>,
}

#[async_trait::async_trait]
impl<F> DelayFunc<F> for TokioDelayFunc<F>
where
...
impl Runtime for TokioRuntime {
  ...
  type DelayFunc<F> = TokioDelayFunc<F> where F: Future + Send + 'static, F::Output: Send;
  ...
  fn delayed_func<F>(&self, delay: Duration, fut: F) -> Self::DelayFunc<F>
  where
    F: Future + Send + 'static,
    F::Output: Send,
  {
    TokioDelayFunc::new(delay, fut)
  }
