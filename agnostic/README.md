...
```toml
# For tokio
agnostic = { version = "0.10", features = ["tokio"] }

# For smol
agnostic = { version = "0.10", features = ["smol"] }
```
...
```toml
# Enable networking
agnostic = { version = "0.10", features = ["tokio", "net"] }

# Enable DNS resolution
agnostic = { version = "0.10", features = ["tokio", "dns"] }

# Enable DNS over HTTPS (ring crypto)
agnostic = { version = "0.10", features = ["tokio", "https-ring"] }

# Enable DNS over QUIC (ring crypto)
agnostic = { version = "0.10", features = ["tokio", "quic-ring"] }

# Enable DNSSEC
agnostic = { version = "0.10", features = ["tokio", "dnssec-ring"] }

# Enable process management
agnostic = { version = "0.10", features = ["tokio", "process"] }

# Enable Quinn QUIC
agnostic = { version = "0.10", features = ["tokio", "quinn"] }
```
...
Each transport protocol requires a crypto backend choice (`ring` or `aws-lc-rs`):

- `tls-ring` / `tls-aws-lc-rs`: DNS over TLS
- `https-ring` / `https-aws-lc-rs`: DNS over HTTPS
- `quic-ring` / `quic-aws-lc-rs`: DNS over QUIC (RFC 9250)
- `h3-ring` / `h3-aws-lc-rs`: DNS over HTTP/3
- `webpki-roots`: Use webpki root certificates
- `rustls-platform-verifier`: Use platform certificate verifier

### DNSSEC Features

- `dnssec-ring`: DNSSEC with ring crypto library
- `dnssec-aws-lc-rs`: DNSSEC with aws-lc-rs crypto library
